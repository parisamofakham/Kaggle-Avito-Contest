...
print("\nEncode Variables")
categorical = ["user_id","region","city","parent_category_name","category_name","user_type","image_top_1","param_1","param_2","param_3","reg_Time_zone"]
...
lgtrain = pd.DataFrame(ridge_oof_train,columns=["deal_probability"],index=traindex)
lgsub = pd.DataFrame(ridge_oof_test,columns=["deal_probability"],index=testdex)
lgtrain.to_csv("lgtrain.csv",index=True,header=True)

lgsub['deal_probability'].clip(0.0005, 0.9995, inplace=True) # Between 0 and 1
